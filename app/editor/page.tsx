...
type DesignState = {
  front: DesignTransform;
  back: DesignTransform;
};

type ResizeHandle = "nw" | "ne" | "sw" | "se";

const DEFAULT_TRANSFORM: DesignTransform = {
  x: 0,
  y: 0,
  size: 120,
};
...
export default function EditorPage() {
  ...
  const [designState, setDesignState] =
    useState<DesignState>(DEFAULT_DESIGN_STATE);
  ...
  const clampTransform = (transform: DesignTransform): DesignTransform => {
    ...
    const minX = -PRINT_AREA.width / 2 + halfW;
    const maxX = PRINT_AREA.width / 2 - halfW;
    const minY = -PRINT_AREA.height / 2 + halfH;
    const maxY = PRINT_AREA.height / 2 - halfH;
  ...
  const updateCurrentSideTransform = (
    ...
    setDesignState((prev) => {
      ...
      const nextTransform = clampTransform(updater(prev[sideKey]));

      return {
        ...prev,
        [sideKey]: nextTransform,
      };
    });
  };
  ...
  const resetCurrentSide = () => {
    if (!hasDesign) return;
    updateCurrentSideTransform(() => ({ ...DEFAULT_TRANSFORM }));
  ...
  const undo = () => {
    ...
    setRedoHistory((prev) => [...prev, designState]);
  ...
  const alignHorizontal = (position: "center") => {
    if (!hasDesign) return;

    updateCurrentSideTransform((prev) => {
      if (position === "center") return { ...prev, x: 0 };
      return prev;
    });
  ...
  const handleResizePointerDown =
    (handle: ResizeHandle) => (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      resizeStartRef.current = {
        startClientX: e.clientX,
        startClientY: e.clientY,
        startSize: currentTransform.size,
        handle,
      };
  ...
  useEffect(() => {
    const handlePointerMove = (e: PointerEvent) => {
      ...
      if (activeResizeHandle && resizeStartRef.current) {
        ...
        const nextSize = resizeStartRef.current.startSize + sizeDelta;
  ...
  return (
    <div
      ...
      onClick={() => {
        if (selectedDesign) setSelectedDesign(false);
      }}
    >
      <style>{`
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{ fontSize: 11, color: "#888" }}>
              Adversarial Clothing Web
            </span>
          </div>
          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            <span style={{ fontSize: 11, color: "#555", cursor: "pointer" }}>
              KR ▾
            </span>
            <Link
              href="/login"
          ...
          >
            {[
              ...
              <>
                ...
                />
                <line key="b2" x1="3" y1="6" x2="21" y2="6" />
                <path key="b3" d="M16 10a4 4 0 0 1-8 0" />
              </>,
              <>
                <path
                  key="u1"
      ...
      >
        ...
        >
          {toolbarItems.map((t, i) => (
            <button
              key={i}
              className={`tool-btn${t.disabled ? " disabled" : ""}`}
              onClick={() => handleToolbarAction(t.label)}
              type="button"
            >
        ...
        >
          ...
          >
            ...
            >
              {(["앞면", "뒷면"] as const).map((side) => (
                <button
                  ...
                  style={{
                    background: viewSide === side ? "#111" : "#fff",
                    color: viewSide === side ? "#fff" : "#333",
            ...
            >
              <img
                src={tshirtImageSrc}
                alt={viewSide === "앞면" ? "tshirt front" : "tshirt back"}
                style={{
                  width: 360,
                  height: "auto",
                  objectFit: "contain",
              ...
              <div
                ...
                onClick={() => {
                  if (!uploadedImg) setSelectedDesign(false);
                }}
              >
                {uploadedImg ? (
                  <div
                    ...
                    style={{
                      ...
                      left: "50%",
                      top: "50%",
                      width: currentTransform.size,
                      height: currentTransform.size,
                      transform: `translate(calc(-50% + ${currentTransform.x}px), calc(-50% + ${currentTransform.y}px))`,
          ...
          >
            <div
              ...
              onClick={() => setActiveTab(activeTab === "상품" ? null : "상품")}
            >
              <svg
                width="22"
                height="22"
            ...
            <div className="right-tool" onClick={() => setActiveTab(null)}>
              <svg
                ...
                viewBox="0 0 24 24"
              >
                <circle cx="13.5" cy="6.5" r=".5" fill="#333" />
          ...
          >
            ...
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>
                색상 – {COLORS.find((c) => c.hex === activeColor)?.name ?? "화이트"}
              </p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {COLORS.map((c) => (
                  <div
                    key={c.hex}
                    className={`color-sw${activeColor === c.hex ? " active" : ""}`}
                    style={{
                      background: c.hex,
                      border: c.border ? "1.5px solid #e0e0e0" : "none",
                    }}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                {SIZES.map((s) => (
                  <button
                    key={s.label}
                    className={`size-btn${activeSize === s.label ? " active" : ""}`}
